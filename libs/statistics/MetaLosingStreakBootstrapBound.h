...
namespace mkc_timeseries
{
  ...
  public:
    struct Options {
      ...
      // --- Optional ACF-driven block length estimation (trade-level, loss-indicator ACF) ---
	...
	seeds[i] = rng_utils::get_random_value(m_rng);
      ...
      concurrency::parallel_for_chunked(
					...
					[&](uint32_t b) {
					  // Per-replicate RNG seeded deterministically from seeds[b]
					  Rng localRng; localRng.seed(seeds[b]);

					  // Resample m trades (optionally using an ACF-chosen L if Sampler supports it)
					  std::vector<Decimal> boot; boot.reserve(m);
					  if (L_auto.has_value()) {
					    if constexpr (std::is_constructible_v<Sampler, std::size_t>) {
					      Sampler localSampler(static_cast<std::size_t>(*L_auto));
					      localSampler.sample(pnl, boot, localRng, m);
					    } else {
