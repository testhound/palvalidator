// BasicBootstrapIntervalTypeTest.cpp
//
// Unit tests for IntervalType functionality in BasicBootstrap
...
std::vector<DecimalType> createTestDataBB()
{
...
TEST_CASE("BasicBootstrap: ONE_SIDED_UPPER basic functionality",
          "[Bootstrap][Basic][IntervalType][OneSidedUpper]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb(B, CL, res, IntervalType::ONE_SIDED_UPPER);
    ...
    SECTION("Lower bound is very low (effectively unbounded)") {
        ...
        // For ONE_SIDED_UPPER, lower bound should be very low
        // (uses q(~100%) in reverse formula: 2*mean - q_max)
        REQUIRE(lb <= mean);
    }
}

TEST_CASE("BasicBootstrap: ONE_SIDED_UPPER different confidence levels",
          "[Bootstrap][Basic][IntervalType][OneSidedUpper]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    SECTION("CL=0.90") {
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb(B, 0.90, res, IntervalType::ONE_SIDED_UPPER);
    ...
    SECTION("CL=0.95") {
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb(B, 0.95, res, IntervalType::ONE_SIDED_UPPER);
    ...
    SECTION("CL=0.99") {
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb(B, 0.99, res, IntervalType::ONE_SIDED_UPPER);
...
TEST_CASE("BasicBootstrap: ONE_SIDED_UPPER higher CL increases upper bound",
          "[Bootstrap][Basic][IntervalType][OneSidedUpper]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb90(B, 0.90, res, IntervalType::ONE_SIDED_UPPER),
        bb95(B, 0.95, res, IntervalType::ONE_SIDED_UPPER),
        bb99(B, 0.99, res, IntervalType::ONE_SIDED_UPPER);
    
    auto r90 = bb90.run(x, sampler, rng90);
    auto r95 = bb95.run(x, sampler, rng95);
    auto r99 = bb99.run(x, sampler, rng99);
...
TEST_CASE("BasicBootstrap: ONE_SIDED_LOWER basic functionality",
          "[Bootstrap][Basic][IntervalType][OneSidedLower]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb(B, CL, res, IntervalType::ONE_SIDED_LOWER);
    ...
    SECTION("Upper bound is very high (effectively unbounded)") {
        ...
        // For ONE_SIDED_LOWER, upper bound should be very high
        // (uses q(~0%) in reverse formula: 2*mean - q_min)
        REQUIRE(ub >= mean);
    }
}

TEST_CASE("BasicBootstrap: ONE_SIDED_LOWER higher CL decreases lower bound",
          "[Bootstrap][Basic][IntervalType][OneSidedLower]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb90(B, 0.90, res, IntervalType::ONE_SIDED_LOWER),
        bb95(B, 0.95, res, IntervalType::ONE_SIDED_LOWER),
        bb99(B, 0.99, res, IntervalType::ONE_SIDED_LOWER);
...
TEST_CASE("BasicBootstrap: ONE_SIDED_UPPER vs TWO_SIDED comparison",
          "[Bootstrap][Basic][IntervalType][Comparison]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb_two(B, CL, res, IntervalType::TWO_SIDED),
        bb_one(B, CL, res, IntervalType::ONE_SIDED_UPPER);
    
    auto result_two = bb_two.run(x, sampler, rng_two);
    auto result_one = bb_one.run(x, sampler, rng_one);
    ...
    SECTION("ONE_SIDED_UPPER upper bound is less conservative") {
        ...
        // ONE_SIDED_UPPER uses pu=95%
        // TWO_SIDED uses pu=97.5%
        // In reverse formula: upper = 2*mean - q_lower
        // So ONE_SIDED should have tighter upper bound
    ...
    SECTION("ONE_SIDED_UPPER lower bound is less constrained") {
        ...
        // ONE_SIDED_UPPER lower at ~minimum of bootstrap
        // TWO_SIDED lower at 2.5th percentile
        REQUIRE(lb_one <= lb_two + 1.0);
    }
}

TEST_CASE("BasicBootstrap: ONE_SIDED_LOWER vs TWO_SIDED comparison",
          "[Bootstrap][Basic][IntervalType][Comparison]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb_two(B, CL, res, IntervalType::TWO_SIDED),
        bb_one(B, CL, res, IntervalType::ONE_SIDED_LOWER);
    ...
    SECTION("ONE_SIDED_LOWER lower bound is higher (less conservative)") {
        ...
        // ONE_SIDED_LOWER uses pl=5%
        // TWO_SIDED uses pl=2.5%
        // In reverse formula: lower = 2*mean - q_upper
        // So ONE_SIDED should have higher lower bound
        REQUIRE(lb_one >= lb_two - 1.0);
    }
    ...
    SECTION("ONE_SIDED_LOWER upper bound is less constrained") {
        ...
        // ONE_SIDED_LOWER upper at ~maximum of bootstrap
...
TEST_CASE("BasicBootstrap: Default IntervalType is TWO_SIDED",
          "[Bootstrap][Basic][IntervalType][BackwardCompatibility]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    // Explicit TWO_SIDED
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb_explicit(B, CL, res, IntervalType::TWO_SIDED);
    ...
    auto result_default = bb_default.run(x, sampler, rng_default);
    auto result_explicit = bb_explicit.run(x, sampler, rng_explicit);
...
TEST_CASE("BasicBootstrap: All three interval types on same data",
          "[Bootstrap][Basic][IntervalType][Comprehensive]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb_two(B, CL, res, IntervalType::TWO_SIDED),
        bb_lower(B, CL, res, IntervalType::ONE_SIDED_LOWER),
        bb_upper(B, CL, res, IntervalType::ONE_SIDED_UPPER);
    
    auto result_two = bb_two.run(x, sampler, rng_two);
    auto result_lower = bb_lower.run(x, sampler, rng_lower);
    auto result_upper = bb_upper.run(x, sampler, rng_upper);
...
// ==================== Reverse Formula Verification ====================

TEST_CASE("BasicBootstrap: Reverse formula is applied correctly",
          "[Bootstrap][Basic][IntervalType][ReverseFormula]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
        bb(B, CL, res, IntervalType::TWO_SIDED);
    ...
    SECTION("Reverse formula relationship") {
        const double mean = num::to_double(result.mean);
        const double lb = num::to_double(result.lower);
        const double ub = num::to_double(result.upper);
        
        // Basic Bootstrap uses: lower = 2*mean - q_upper, upper = 2*mean - q_lower
        // This means: mean should be between lower and upper (but not necessarily centered)
        REQUIRE(lb <= mean);
        REQUIRE(mean <= ub);
        
        // The interval should be "reflected" around mean
        // So we can compute the implied quantiles
        const double q_implied_upper = 2.0 * mean - lb;  // Should be an upper bootstrap quantile
        const double q_implied_lower = 2.0 * mean - ub;  // Should be a lower bootstrap quantile
        
        // Verify ordering
        REQUIRE(q_implied_lower <= mean);
        REQUIRE(mean <= q_implied_upper);
        
        // Get bootstrap distribution for verification
        const auto& theta_star = bb.getBootstrapStatistics();
        REQUIRE(!theta_star.empty());
        
        // Implied quantiles should be within the bootstrap distribution range
        const double min_theta = *std::min_element(theta_star.begin(), theta_star.end());
        const double max_theta = *std::max_element(theta_star.begin(), theta_star.end());
        
        REQUIRE(q_implied_lower >= min_theta - 1.0);  // Allow small tolerance
        REQUIRE(q_implied_upper <= max_theta + 1.0);  // Allow small tolerance
    }
}

// ==================== Diagnostics Test ====================

TEST_CASE("BasicBootstrap: IntervalType with diagnostics",
          "[Bootstrap][Basic][IntervalType][Diagnostics]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    SECTION("Diagnostics are available after run") {
        ...
        double mean_boot = bb.getBootstrapMean();
        double var_boot = bb.getBootstrapVariance();
        double se_boot = bb.getBootstrapSe();
...
TEST_CASE("BasicBootstrap: IntervalType with ThreadPoolExecutor",
          "[Bootstrap][Basic][IntervalType][ThreadPool]")
{
    using D = DecimalType;
    ...
    > bb_single(B, CL, res, IntervalType::ONE_SIDED_UPPER);
    
    // Thread-pooled
    BasicBootstrap<
        D, MeanSamplerIT, StationaryMaskValueResampler<D>,
        std::mt19937_64, concurrency::ThreadPoolExecutor<4>
    > bb_pool(B, CL, res, IntervalType::ONE_SIDED_UPPER);
...
TEST_CASE("BasicBootstrap: IntervalType with skewed data",
          "[Bootstrap][Basic][IntervalType][EdgeCases]")
{
    using D = DecimalType;
    ...
    SECTION("Handles skewed data correctly") {
        ...
        // For skewed data, Basic Bootstrap should handle asymmetry
        const double mean = num::to_double(result.mean);
        const double ub = num::to_double(result.upper);
        REQUIRE(ub > mean);  // Upper bound should be above mean
    }
}

TEST_CASE("BasicBootstrap: IntervalType with negative values",
          "[Bootstrap][Basic][IntervalType][EdgeCases]")
{
    using D = DecimalType;
...
TEST_CASE("BasicBootstrap: IntervalType with move semantics",
          "[Bootstrap][Basic][IntervalType][Move]")
{
    using D = DecimalType;
    auto x = createTestDataBB();
    ...
    SECTION("Move constructor preserves IntervalType") {
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb1(B, CL, res, IntervalType::ONE_SIDED_UPPER);
        
        randutils::seed_seq_fe128 seed{123u, 456u};
        std::mt19937_64 rng(seed);
        
        // Run on bb1
        auto result1 = bb1.run(x, sampler, rng);
        
        // Move construct bb2 from bb1
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb2(std::move(bb1));
        
        // bb2 should have diagnostics from bb1
        REQUIRE(bb2.hasDiagnostics());
        
        // Verify diagnostics are preserved
        auto stats = bb2.getBootstrapStatistics();
        REQUIRE(stats.size() == result1.effective_B);
    }
    
    SECTION("Move assignment preserves IntervalType") {
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb1(B, CL, res, IntervalType::ONE_SIDED_LOWER);
        
        BasicBootstrap<D, MeanSamplerIT, StationaryMaskValueResampler<D>>
            bb2(B, 0.90, res, IntervalType::TWO_SIDED);  // Different config
        
        randutils::seed_seq_fe128 seed{789u, 101u};
        std::mt19937_64 rng(seed);
        
        // Run on bb1
        auto result1 = bb1.run(x, sampler, rng);
        
        // Move assign bb2 from bb1
        bb2 = std::move(bb1);
