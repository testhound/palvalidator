...
template <int Prec>
class TestTradingPositionObserver : public TradingPositionObserver<Prec>
{
public:
  TestTradingPositionObserver()
    : TradingPositionObserver<Prec>(),
      mPositionClosed(false),
      mExitPrice(DecimalConstants<Prec>::DecimalZero)
  {}

  const decimal<Prec>& getExitPrice() const
  {
    return mExitPrice;
  }
...
TEST_CASE ("TradingPosition operations", "[TradingPosition]")
{
  ...
  TradingPositionLong<7> longPosition1(tickerSymbol, entry0->getOpenValue(),  entry0, oneContract);
  ...
  DecimalType oneHundred(DecimalConstants<7>::DecimalOneHundred);

  DecimalType longRefReturn (((lastClose1 - longEntry1)/longEntry1));
  DecimalType longRefPercentReturn (longRefReturn * oneHundred);
  DecimalType longRefMultiplier(longRefReturn + DecimalConstants<7>::DecimalOne);
  ...
  TradingPositionShort<7> shortPosition1(tickerSymbol, shortEntry0->getOpenValue(),  shortEntry0, oneContract);
  ...
  DecimalType shortRefMultiplier(shortRefReturn + DecimalConstants<7>::DecimalOne);
  ...
  TradingPositionShort<7> shortPosition2(tickerSymbol, shortEntry10->getOpenValue(),  shortEntry10, oneContract);
  ...
  SECTION ("TradingPositionLong close observer test")
  {
    ...
    TestTradingPositionObserver<7> aObserver;
  ...
  SECTION ("TradingPositionShort close position test w R multiple")
  {
    ...
    PercentNumber<7> stopInPercent (PercentNumber<7>::createPercentNumber(stopInDecimal));
...
SECTION ("TradingPositionShort close position test 2 with R multiple")
  ...
  {
    TradingPosition<7>::ConstPositionBarIterator it = longPosition1.beginPositionBarHistory();
    it++;
  ...
  SECTION ("TradingPositionShort ConstIterator after position closes tests")
  {
    ...
    TradingPosition<7>::ConstPositionBarIterator it = shortPosition1.beginPositionBarHistory();
    it++;
