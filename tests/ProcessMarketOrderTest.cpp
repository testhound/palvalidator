...
TEST_CASE ("ProcessMarketOrderTest Market Order Operations", "[ProcessOrderVisitor]")
{
  ...
  SECTION ("Verify long orders are executed")
  {
    ...
    REQUIRE (longOrder1.getFillPrice() == entry0->getOpenValue());
    REQUIRE (longOrder1.getFillDate() == entry0->getDateValue());
  }
  ...
  SECTION ("Verify short orders are executed")
  {
    ...
    REQUIRE (shortOrder1.isOrderExecuted() == true);
    REQUIRE (shortOrder1.isOrderPending() == false);
    REQUIRE (shortOrder1.isOrderCanceled() == false);
    REQUIRE (shortOrder1.getFillPrice() == shortEntry0->getOpenValue());
    REQUIRE (shortOrder1.getFillDate() == shortEntry0->getDateValue());
  }
